...
- [x] Плавающая кнопка `Добавить` закреплена по центру нижней панели (`FabPosition.Center`) для соответствия mobile-first макету.
...
- [x] Календарь дополнен секцией «Выбранная дата»: выбор дня из списка дат и empty-state при отсутствии операций.
- [x] На экране добавления операции в header добавлен правый пустой слот для визуального баланса с кнопкой «Отмена».

- [x] Унифицировано отображение валюты: суммы на dashboard, в календаре и в последних операциях теперь форматируются через единый `toRubCurrency()` с локалью `ru-RU`.
- [x] Улучшена доступность формы добавления операции: для поля `Сумма` включена цифровая клавиатура (`KeyboardType.Number`).
- [x] Добавлены состояния загрузки для dashboard и календаря: при старте показывается `CircularProgressIndicator` до первичной инициализации данных.
- [x] Экран настроек детализирован по блокам из плана: выделены секции «Тема и синхронизация», «Семейный доступ» и «Общие настройки» с явным действием выхода.

- [x] Экран аналитики переведён на данные `CoinMetricViewModel`: распределение расходов и прогресс лимитов теперь считаются по реальным последним операциям.
- [x] Для аналитики добавлено состояние загрузки и empty-state, чтобы экран корректно отрабатывал до инициализации данных и при отсутствии расходных операций.
- [x] Улучшена доступность формы добавления операции: поля `Сумма` и `Категория` визуально подсвечиваются как ошибочные при невалидной отправке.
- [x] Валидация формы добавления операции детализирована: ошибки `Сумма` и `Категория` теперь показываются адресно под каждым полем с отдельными сообщениями.
- [x] Плавающая кнопка `Добавить` в нижней панели сделана круглее и больше остальных кнопок навигации.
- [x] Экран календаря теперь представляет собой полноценный календарь с возможностью просмотра транзакций по дням.
- [x] Добавлена возможность редактирования существующих транзакций.
- [x] Кнопка отмены при добавлении транзакции сделана более заметной в виде красного значка.
- [x] В поле суммы при добавлении транзакции добавлен значок калькулятора, который позволяет вычислять выражения и добавлять результат в сумму.
